...
```
And so on and so on with any type of Monoalphabetic Cipher you can think of. With some slight modifications we can even convert Morse Code back to English! Use `invert` to invert our `Morse_code` object/hash:
```ruby
