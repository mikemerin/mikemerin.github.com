...
1) Find the username of a github user from the URL
...
The magic here is our `([^\/]*)`. Breaking it down: `\/` is our backslash search, it's in [hard brackets] and starts with a carrot `^` to say "search for all characters that aren't a backslash", and finally our asterisk `*` after the hard brackets say "find as many of them as possible, but if you don't find any that's fine, still output all the non-backslash characters."
