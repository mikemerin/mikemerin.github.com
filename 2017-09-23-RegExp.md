...
`.some` is basically `.forEach` (`.each` in Ruby) that breaks after an answer is found. As you can see, the regex script `"string".match(/s/)` does all of these, plus also takes in the index, the initial string, and much more.
...
and that will return any part of the string that matches.

```javascript
"hello world".match(/(hello)/)
// [ 'hello', 'hello', index: 0, input: 'hello world' ]

// The second 'hello' is the word that's found in the parenthesis
...
Anything found in the resulting array after the first element (which is what's matched) will be what you find in the parenthesis(es)
