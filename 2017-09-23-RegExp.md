...
Matching one character is fine, but what if we wanted to find the first vowel in a string? We'd say "find the letter `a` or `e` or `i` etc.". If we enclose characters in `[hard brackets]` it will return whichever of those characters found first:
...
What if we want to match a lowercase letter? An uppercase letter? A range of numbers or letters? Regex makes that easy as well:
...
// see next section for how to do multi-digit ranges
```
